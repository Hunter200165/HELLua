...
local Punctuation = '!@#%$%%%^:&%?%*=%-%+/|~<>';
local PunctuationSet = ('[%s]'):format(Punctuation);
local NoPunctuationSet = ('([^%s])'):format(Punctuation);
local NoPunctuationSetF = NoPunctuationSet:gsub('%%', '%%%%');
...
function TLua:ProcessText(Text)
	...
	self.Comments = TList.Create();

	local Strings = self.Strings;
	local Comments = self.Comments;
	...
	local PassMacros = function(Current, Macros)
		...
		TLua.ParseStringForPatterns(Result, Current, function(Start, Finish)
			...
			local NFinish, ResString = MRFunc(Macros, Result:sub(Start, Finish), Current, Result, Start, Finish);
...
local Macro_NotBoolC = TMacros.New(
	NoPunctuationSetF + '!([^=])',
	function(self, str, pattern)
		return str:gsub(pattern, '%1 not %2');
	end,
	function(Str, Warn, IsValidPos, Machine)
		local Msg = 'Syntax Error: "!" operator is not compiled at [[%d]] - ' + Check_VarsMsg;
		local Msk = '[^%p]![^%p]';
...
local Macro_NotBitC = TMacros.New(
	HELLua.IterateAll(ValueOrFunction, NoPunctuationSetF + '~%%s-%s', 1),
	function(self, str, pattern)
		return str:gsub(pattern, '%1(bit%.bnot%(%2%))');
	end,
	function(Str, Warn, IsValidPos, Machine)
		local Msg = 'Syntax Error: "~" operator is not compiled at [[%d]] - ' + Check_VarsMsg;
		local Msk = '[^%p]~[^%p]';
...
local Macro_DivPascal = TMacros.New(
	HELLua.IterateAll(ValueOrFunction, '%s%%s+div%%s+%s', 2),
	function(self, str, pattern)
...
local Macro_ModPascal = TMacros.New(
	...
	HELLua.IterateAll(ValueOrFunction, '%s%%s+mod%%s+%s', 2),
	function(self, str, pattern)
		return str:gsub(pattern, '(%1) %% (%2)');
	end
);
...
local Macro_EnhancedTypeCheck_Inversed_C = TMacros.New(
	HELLua.IterateAll(ValueOrFunction, '%s%%s-%%:%%!%%s-(%%w+)', 1),
	function(self, str, pattern)
		local PS, PF, F, S = str:find(pattern);
		S = S:lower();
...
local Macro_EnhancedTypeCast_C = TMacros.New(
	HELLua.IterateAll(ValueOrFunction, '%s%%s-%%:%%>%%s-(%%w+)', 1),
	function(self, str, pattern)
		local PS, PF, F, S = str:find(pattern);
		S = S:lower();
		return str:gsub(pattern, '%(to' + S + '%(%1%)%)');
	end,
	function(Str, Warn, IsValidPos, Machine)
		local Msg = 'Syntax Error: ":>" operator is not compiled at [[%d]] - (var, number, brackets) :? (string). \n';
		local Msk = '[^%p]%:%>[^%p]';
...
local Macro_EnhancedReturnIf = TMacros.New(
	'(%s)return%s-(%b())%s-if%s-(%b())',
	function(self, str, pattern)
		local PS, PF, Space, Returning, Condition = str:find(pattern);
		return ('%s if %s then return %s end '):format(Space, Condition, Returning:sub(2, -2));
	end
); 
...
local Macro_EnhancedGlobal = TMacros.New(
	'([^%w])global([^%w])', 
	function(self, str, pattern)
		return str:gsub(pattern, '%1_G.%2');
	end
);

local Macro_EnhancedNumbericConstants = TMacros.New(
	NoPunctuationSet + '%$%((%d+)%)([%w_]+)', 
	function(self, str, pattern)
		return str:gsub(pattern, function(Symbol, NumBase, Number)
			Number = Number:gsub('_', '');
			return ('%s tonumber("%s", %s)'):format(Symbol, Number, NumBase);
		end);
	end
);

local Macro_EnhancedNumbericBinaryConstants = TMacros.New(
	NoPunctuationSet + '%$([%w_]+)', 
	function(self, str, pattern)
		return str:gsub(pattern, function(Symbol, Number)
			Number = Number:gsub('_', '');
			return ('%s tonumber("%s", 2)'):format(Symbol, Number);
		end);
	end
);

local Macro_EnhancedNotIsNil = TMacros.New(
	HELLua.IterateAll(ValueOrFunction, '%s%%s-?' + NoPunctuationSetF, 1), 
	function(self, str, pattern)
		return str:gsub(pattern, '(%1 ~= nil)%2');
	end
);
...
HELLua.MacrosesEnhanced = TList.New {
	Macro_EnhancedNull;
	Macro_EnhancedNotIsNil;
	Macro_EnhancedNumbericConstants;
	Macro_EnhancedNumbericBinaryConstants;
	...
	Macro_EnhancedTypeCheck_Inversed_C;
	Macro_EnhancedTypeCast;
	Macro_EnhancedTypeCast_C;
	Macro_EnhancedTypeCheckNotC;
	Macro_EnhancedTypeCastAnother;
	Macro_EnhancedUseless;
	Macro_EnhancedGlobal;
	Macro_EnhancedReturnIf;
	-- Macro_EnhancedInterpolateString;
};
...
local Macro_InterpolateString = TMacros.New(
	'%$', 
	function(self, str, pattern, String, Start)
		local Pos = Start + 1;
		local Char = String:sub(Pos, Pos);
		while (Char == ' ') do Pos = Pos + 1; Char = String:sub(Pos, Pos); end;
		if (Char == '"') or (Char == '\'') then 
			local Strings = Machine.Strings;
			local FString = nil;
			local FPos = nil;
			for i = 1, #Strings do 
				local String = Strings[i];
				if String.Start == Pos then 
					FString = String;
					FPos = i;
					break;
				end;
			...
			local STR = String:sub(FString.Start, FString.Finish);
			local Inter = {};
			local InterBr = {};
			-- If not String == %{}
			STR = STR:gsub('%%([^{%[])', '%%%%%1');
			STR = STR:gsub('(.)(%b{})', function(Symbol, Strg)
				if Symbol == '%' then 
					return Strg;
				else
					table.insert(Inter, ('tostring(%s)'):format(Strg:sub(2, -2):gsub('%%%%', '%%')));
					return ('%s%%s'):format(Symbol);
				end; 
			end);
			if #Inter > 0 then 
				return FString.Finish, ('((%s):format(%s))'):format(STR, table.concat(Inter, ', '));
			else 
				return FString.Finish, ('(%s)'):format(STR);
			end;
		else 
			return nil, nil;
		end;
...
HELLua.MacrosesInterpolation = {
	Macro_InterpolateString;
};
